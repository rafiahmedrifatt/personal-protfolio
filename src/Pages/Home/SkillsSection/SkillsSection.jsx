...
const skillPillars = [
    {
        ...
        emoji: '🎨',
        tone: 'from-cyan-500/20 to-cyan-500/10',
    ...
    },
    {
        ...
        emoji: '🚀',
        tone: 'from-green-600/20 to-green-500/10',
...
const SkillsSection = () => {
    return (
        <motion.section
            id='skills'
            className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-900 to-cyan-900 py-14 sm:py-16 lg:py-20"
            initial="hidden"
            whileInView="visible"
        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-14">
                <motion.div className="text-center space-y-4" variants={cardVariants}>
                    <span className='inline-block px-6 py-3 bg-gradient-to-r from-cyan-500/20 to-green-500/20 text-cyan-300 rounded-full text-sm font-medium border border-cyan-500/30 shadow-lg'>
                        🧰 Skillset Overview
                    </span>
                ...
                <motion.div className="grid gap-6 sm:gap-8 md:grid-cols-3" variants={listVariants}>
                    {stats.map((item) => (
                        <motion.div
                            key={item.label}
                            className="rounded-3xl border border-slate-700/50 bg-slate-900/70 p-6 sm:p-7 shadow-xl text-left space-y-3"
                            variants={cardVariants}
                            {...hoverLift}
                        >
                            <p className="text-3xl font-black text-white">{item.value}</p>
                            <p className="text-sm font-semibold uppercase tracking-wide text-cyan-200">{item.label}</p>
                ...
                <div className="grid gap-10 lg:grid-cols-[3fr_2fr] lg:gap-12">
                    <motion.div className="space-y-8" variants={listVariants}>
                        {skillPillars.map((pillar) => (
                            <motion.div
                                key={pillar.title}
                                className={`rounded-3xl border border-slate-700/40 bg-gradient-to-br ${pillar.tone} p-6 sm:p-8 shadow-2xl`}
                                variants={cardVariants}
                                {...hoverLift}
                            >
                                ...
                                <div className="space-y-3">
                                    {pillar.highlights.map((highlight) => (
                                        <motion.div
                                            key={highlight}
                                            className="flex items-start gap-3 rounded-2xl bg-slate-900/50 border border-slate-700/40 px-4 py-3"
                    ...
                    <motion.aside className="space-y-8" variants={listVariants}>
                        <motion.div
                            className="rounded-3xl border border-cyan-500/20 bg-cyan-500/10 p-6 sm:p-7 shadow-xl space-y-4"
                            variants={cardVariants}
                        >
                            ...
                            <div className="flex flex-wrap gap-2">
                                {toolbox.map((tool) => (
                                    <motion.span
                                        key={tool}
                                        className="px-3 py-1.5 rounded-full bg-slate-900/80 border border-slate-700 text-xs font-semibold text-slate-100"
                        ...
                        <motion.div
                            className="rounded-3xl border border-slate-700/40 bg-slate-900/70 p-6 sm:p-7 shadow-xl space-y-4"
                            variants={cardVariants}
                        >
                            ...
                            <div className="space-y-4">
                                {growthAreas.map((area) => (
                                    <motion.div
                                        key={area.title}
                                        className="rounded-2xl border border-slate-700/40 bg-slate-800/60 px-4 py-4"
